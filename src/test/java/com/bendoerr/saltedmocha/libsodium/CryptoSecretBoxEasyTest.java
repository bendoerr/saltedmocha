...
import static com.bendoerr.saltedmocha.libsodium.CryptoSecretBoxEasy.*;
import static org.bouncycastle.util.Arrays.*;
import static org.junit.Assert.*;

public class CryptoSecretBoxEasyTest {
    final byte[] firstkey_1 = new byte[]{
            (byte) 0x1b, (byte) 0x27, (byte) 0x55, (byte) 0x64, (byte) 0x73,
            (byte) 0xe9, (byte) 0x85, (byte) 0xd4, (byte) 0x62, (byte) 0xcd,
            (byte) 0x51, (byte) 0x19, (byte) 0x7a, (byte) 0x9a, (byte) 0x46,
            (byte) 0xc7, (byte) 0x60, (byte) 0x09, (byte) 0x54, (byte) 0x9e,
            (byte) 0xac, (byte) 0x64, (byte) 0x74, (byte) 0xf2, (byte) 0x06,
            (byte) 0xc4, (byte) 0xee, (byte) 0x08, (byte) 0x44, (byte) 0xf6,
            (byte) 0x83, (byte) 0x89};

    final byte[] nonce_1 = new byte[]{
    ...
    final byte[] c_expected = new byte[]{
    ...
    @Test
    public void test_secretbox_easy_a() throws Exception {
        System.out.println("libsodium/test/default/secretbox_easy.c");

        byte[] c = new byte[m_1.length + crypto_secretbox_MACBYTES];

        crypto_secretbox_easy(c, m_1, nonce_1, firstkey_1);
        assertArrayEquals(c_expected, c);

        byte[] m2 = new byte[m_1.length];
        crypto_secretbox_open_easy(m2, c, nonce_1, firstkey_1);
        assertArrayEquals(m_1, m2);
    }

    @Test
    public void test_secretbox_easy_b() throws Exception {
        System.out.println("libsodium/test/default/secretbox_easy.c");

        byte[] c = new byte[m_1.length];
        byte[] mac = new byte[crypto_secretbox_MACBYTES];

        crypto_secretbox_detached(c, mac, m_1, nonce_1, firstkey_1);
        assertArrayEquals(c_expected, concatenate(mac, c));
    }

    @Test
    public void test_secretbox_easy_c() throws Exception {
        System.out.println("libsodium/test/default/secretbox_easy.c");

        byte[] c = new byte[m_1.length + crypto_secretbox_MACBYTES];
        byte[] mac = new byte[crypto_secretbox_MACBYTES];

        System.arraycopy(m_1, 0, c, 1, 131);
        crypto_secretbox_easy(c, copyOfRange(c, 1, 132), nonce_1, firstkey_1);
        assertArrayEquals(c_expected, c);
    }

    @Test
    public void test_secretbox_easy_d() throws Exception {
        System.out.println("libsodium/test/default/secretbox_easy.c");
    ...
    @Test
    public void test_secretbox_easy2_a() throws Exception {
        System.out.println("libsodium/test/default/secretbox_easy2.c");
        Random r = new SecureRandom();

        int mlen = r.nextInt(10000);
        byte[] m = new byte[mlen];
        byte[] m2 = new byte[mlen];
        byte[] c = new byte[mlen + crypto_secretbox_MACBYTES];
        byte[] nonce = new byte[crypto_secretbox_NONCEBYTES];
        byte[] k = new byte[crypto_secretbox_KEYBYTES];
        byte[] mac = new byte[crypto_secretbox_MACBYTES];
        ...
        crypto_secretbox_easy(c, m, nonce, k);
        crypto_secretbox_open_easy(m2, c, nonce, k);
        assertArrayEquals(m, m2);
    }

    @Test
    public void test_secretbox_easy2_b() throws Exception {
        System.out.println("libsodium/test/default/secretbox_easy2.c");
        Random r = new SecureRandom();
        ...
        crypto_secretbox_detached(c, mac, m, nonce, k);
        crypto_secretbox_open_detached(m2, c, mac, nonce, k);
        assertArrayEquals(m, m2);
    }
